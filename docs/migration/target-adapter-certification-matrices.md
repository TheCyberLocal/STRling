...
P18-T02 will derive one deterministic machine evidence family and one
documentation-ready projection from governed structured results. Every cell
will name an exact engine/profile/version or adapter/package/runtime/platform
coordinate, its tier-appropriate obligation, an explicit status, evidence
links and fingerprints, and the last certified commit. Missing or stale cells
will fail closed when profiles, corpus versions, adapters, package coordinates,
or certification tiers change.
