...
Status: P17-T06 CP2 verification design and evidence
...
The post-migration public extractors remain binding-specific views over small
facades, never semantic authorities:

-   Go retains the governed `go-doc-declarations` extraction and the existing
    JSON snapshot shape. CP4 must reproduce it with Go `>=1.22,<1.23` after the
    facade is in place.
-   Dart will activate an analyzer-backed export-reachability extractor under
    SDK `>=3.0,<4.0`. It records exported libraries, classes, constructors,
    members, parameters, nullability, and stable exception identities in the
    common public-contract snapshot envelope.
-   Swift will activate a normalized symbol-graph extractor under Swift
    `>=5.9,<7.0`. It records the `STRling` product/module, public declarations,
...
extractors and toolchain constraints are generator inputs. A missing extractor
or toolchain keeps a surface transitional and cannot be converted into a
passing snapshot row.

The final Essential/Simply helper surfaces are generated by
