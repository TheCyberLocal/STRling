...
use crate::editor_intelligence::{
    project_semantic_rewrite_actions, EditorCaptureLink, EditorCompletion, EditorCompletionTier,
...
pub(crate) fn project_editor(
    ...
    let rewrite_actions = parsed.as_ref().map_or_else(Vec::new, |parsed| {
        project_semantic_rewrite_actions(document, text, &parsed.program, &node_spans)
    });
    FrontendEditorEvidence {
