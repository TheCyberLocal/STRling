...
use strling_kernel::python_re_lowering::{
    lower_python_re, PythonReLoweringErrorCode, MAX_PYTHON_RE_LOWERING_DEPTH,
};
...
fn artifact(semantic: &SemanticProgram, profile: &TargetProfile) -> Result<TargetArtifact, String> {
    ...
    let lowered = lower_python_re(semantic, profile, &plan).map_err(|error| error.to_string())?;
    serialize_python_re(&lowered).map_err(|error| error.to_string())
}
...
fn pathological_serializer_inputs_are_bounded_deterministic_and_panic_free() {
    ...
    let expected_pathologies = BTreeSet::from([
        ...
        "str_and_bytes_profile_confusion",
    ]);
    ...
    for profile in [str_target(), bytes_target()] {
        let empty = artifact(
        ...
        assert!(empty.pattern.flags.is_empty());
    }

    let profile = str_target();
    ...
    .expect("large bounded quantifier artifact");
    ...
    let legal_capture = artifact(
        &program(json!({
            ...
            "name": "capture_legal_32_ascii_units",
            "body": literal("node:pathology.capture.legal.body", "a")
        })),
        &profile,
    )
    .expect("legal capture name");
    assert!(legal_capture
        .pattern
        .text
        .contains("capture_legal_32_ascii_units"));

    let invalid = program(json!({
        "node_id": "node:pathology.capture.invalid",
        "kind": "capture",
        "capture_id": "capture:pathology.invalid",
        "name": "bad-name",
    ...
    deep.root = root;
    assert_eq!(
        lower_python_re(&deep, &profile, &shallow_plan)
    ...
    let mut malformed =
        lower_python_re(&capture_program, &profile, &capture_plan).expect("capture lowering");
    malformed.captures[0].slot = 2;
    assert_eq!(
        serialize_python_re(&malformed)
    ...
    let mut conflicting = lower_python_re(&plain, &profile, &plain_plan).expect("plain lowering");
    conflicting.options.push(conflicting.options[0].clone());
    assert_eq!(
        serialize_python_re(&conflicting)
            .expect_err("conflicting options")
            .code,
        PythonReSerializationErrorCode::InvalidLoweringPlan
    );

    let bytes = bytes_target();
    let bytes_plan = plan_for(&plain, &bytes).expect("bytes plan");
    let mut confused = lower_python_re(&plain, &bytes, &bytes_plan).expect("bytes lowering");
    confused.target_profile = profile.reference().expect("str profile reference");
    assert_eq!(
        serialize_python_re(&confused)
            .expect_err("cross-profile artifact")
            .code,
    ...
    program(json!({
        ...
        "items": [
            ...
            },
            {
                ...
                "kind": "atomic",
                "body": {
                    "node_id": "node:performance.atomic.choice",
                    "kind": "alternation",
                    "branches": [
                        literal("node:performance.atomic.left", "ab"),
                        literal("node:performance.atomic.right", "a")
                    ]
                }
            },
...
fn compiler_stage_medians_remain_within_governed_baseline_budgets() {
    ...
    for _ in 0..sample_count {
        ...
        let started = Instant::now();
        let lowered = lower_python_re(&semantic, &profile, &plan).expect("lowering");
        lowering_samples.push(started.elapsed());

        let started = Instant::now();
        let emitted = serialize_python_re(&lowered).expect("serialization");
