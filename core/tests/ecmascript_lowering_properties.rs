...
use strling_kernel::ecmascript_lowering::{lower_ecmascript, EcmascriptLoweringErrorCode};
...
fn generated_valid_programs_lower_deterministically_without_input_mutation() {
    ...
    for seed in 0..196 {
        ...
        let first = lower_ecmascript(&semantic, &target, &portability).expect("first lowering");
        let second = lower_ecmascript(&semantic, &target, &portability).expect("second lowering");
...
fn generated_cross_program_evidence_is_always_rejected() {
    ...
    for seed in 0..64 {
        ...
        let failure = lower_ecmascript(&supplied, &target, &portability)
