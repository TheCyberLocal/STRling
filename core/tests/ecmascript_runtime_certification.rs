...
use strling_kernel::ecmascript_lowering::{
    lower_ecmascript, EcmascriptLoweringErrorCode, MAX_ECMASCRIPT_LOWERING_DEPTH,
};
...
fn artifact(semantic: &SemanticProgram, profile: &TargetProfile) -> Result<TargetArtifact, String> {
    ...
    let lowered = lower_ecmascript(semantic, profile, &plan).map_err(|error| error.to_string())?;
    serialize_ecmascript(&lowered).map_err(|error| error.to_string())
}

fn runtime_corpus() -> Value {
    let path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../tests/conformance/ecmascript-runtime-certification.json");
...
fn pathological_serializer_inputs_are_bounded_deterministic_and_panic_free() {
    ...
    assert_eq!(vec!["u"], empty.pattern.flags);
    ...
    deep.root = root;
    assert_eq!(
        lower_ecmascript(&deep, &profile, &shallow_plan)
    ...
    let mut malformed =
        lower_ecmascript(&capture_program, &profile, &capture_plan).expect("capture lowering");
    malformed.captures[0].slot = 2;
    assert_eq!(
        serialize_ecmascript(&malformed)
    ...
    let mut conflicting = lower_ecmascript(&plain, &profile, &plain_plan).expect("plain lowering");
    conflicting.options.push(conflicting.options[0].clone());
    assert_eq!(
        serialize_ecmascript(&conflicting)
    ...
    program(json!({
        ...
        "items": [
            ...
            },
            {
                ...
                "kind": "atomic",
                "body": literal("node:performance.atomic.literal", "a")
            },
...
fn compiler_stage_medians_remain_within_governed_baseline_budgets() {
    ...
    for _ in 0..sample_count {
        ...
        let started = Instant::now();
        let lowered = lower_ecmascript(&semantic, &profile, &plan).expect("lowering");
        lowering_samples.push(started.elapsed());

        let started = Instant::now();
        let emitted = serialize_ecmascript(&lowered).expect("serialization");
