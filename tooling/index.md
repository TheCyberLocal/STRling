...
-   `performance_resource_certification.py` — Validates the performance/resource
    ...
    native Windows requires exact hard affinity plus CPU-set placement, fixed
    frequency and HighQoS policy, unlimited quota, authenticated allocation
    state, and a passing one-shot quiescence observation. Local and Pull Request
    remain offline.
-   `performance_windows.py` — Applies and authenticates native Windows process
    affinity and CPU sets; records allocation state, CPU topology, job quota,
    power and timer identity; rejects guest firmware or a CPU Set reserved to a
    different process; and emits bounded pre-measurement quiescence evidence
    without retrying or modifying the host.
