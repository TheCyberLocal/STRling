...
class DynamicLanguageAdapterCertificationTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.schema = certification._schema()
        cls.manifest = certification._build_manifest()
        cls.baseline = certification._build_baseline()
    ...
    def test_repository_documents_certify(self) -> None:
        report = certification.DynamicLanguageAdapterCertificationSuite().certify()
        self.assertEqual(report.case_count, 129)
        self.assertEqual(report.public_input_count, 28)
        self.assertEqual(report.semantic_copy_count, 83)
        self.assertEqual(report.historical_source_count, 157)

    def test_case_removal_fails_after_editing_declared_total(self) -> None:
        manifest = copy.deepcopy(self.manifest)
        manifest["cases"].pop()
        manifest["counts"]["total"] = 128
        manifest["fingerprint"] = certification._fingerprint_json(manifest)
        with self.assertRaises(certification.DynamicLanguageAdapterCertificationError):
    ...
    def test_support_disposition_cannot_be_promoted(self) -> None:
        manifest = copy.deepcopy(self.manifest)
        manifest["provisional_dispositions"][0]["status"] = "supported"
        manifest["fingerprint"] = certification._fingerprint_json(manifest)
        with self.assertRaises(certification.DynamicLanguageAdapterCertificationError):
            self.certify(manifest=manifest)
