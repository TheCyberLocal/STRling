...
class JvmAdapterCertificationTests(unittest.TestCase):
    @classmethod
    ...
    ) -> certification.JvmAdapterCertificationReport:
        return certification.JvmAdapterCertificationSuite().certify_documents(
            self.schema,
    ...
    def test_repository_documents_certify(self) -> None:
        report = certification.JvmAdapterCertificationSuite().certify()
        self.assertEqual(report.case_count, 72)
        self.assertEqual(report.semantic_copy_count, 35)
        self.assertEqual(report.historical_source_count, 70)
    ...
    def test_case_removal_fails_after_editing_declared_total(self) -> None:
        ...
        with self.assertRaises(certification.JvmAdapterCertificationError):
    ...
    def test_case_identity_substitution_fails(self) -> None:
        ...
        with self.assertRaisesRegex(
            certification.JvmAdapterCertificationError, "required JVM evidence"
        ):
            self.certify(manifest)
    ...
    def test_runner_substitution_fails(self) -> None:
        ...
        with self.assertRaisesRegex(
            certification.JvmAdapterCertificationError, "runner denominator"
        ):
            self.certify(manifest)
    ...
    def test_observation_reclassification_fails(self) -> None:
        ...
        with self.assertRaisesRegex(
            certification.JvmAdapterCertificationError, "observations changed"
        ):
            self.certify(manifest)

    def test_semantic_path_substitution_fails_after_fingerprint_edit(self) -> None:
        baseline = copy.deepcopy(self.baseline)
        baseline["semantic_copy_files"][0]["path"] = "bindings/java/pom.xml"
        baseline["fingerprint"] = certification._fingerprint_json(
            baseline, {"fingerprint"}
        )
        with self.assertRaisesRegex(
            certification.JvmAdapterCertificationError, "does not reproduce"
        ):
