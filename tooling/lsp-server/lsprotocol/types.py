...
class Diagnostic:
    ...
    # Auxiliary, server-defined payload preserved across the
    # diagnostic→codeAction round-trip per LSP § Diagnostic.data.
...
@dataclass
class WorkspaceEdit:
    # ``changes`` maps document URI → list of TextEdit, matching the LSP
    # ``WorkspaceEdit.changes`` shape exactly.
    changes: dict
...
# --- Semantic Tokens ------------------------------------------------------- #
# Minimal shapes required by the STRling language server. The real
# `lsprotocol` package ships richer typed dataclasses; we only need enough
# surface to satisfy the in-process call sites and tests.
