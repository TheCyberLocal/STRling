"""Minimal local ``pygls``-compatible transport used by STRling.

This shim exists because the repository vendors a ``pygls`` package name
inside ``tooling/lsp-server/``. Earlier revisions left the transport
methods as stubs, which meant the language server process exited without
ever reading initialize/didOpen/didChange messages from VS Code.

The implementation below keeps the tiny surface area the tests expect
(``feature`` registration and introspection) but now runs a working
stdio/TCP JSON-RPC loop, publishes diagnostics, and answers initialize
with the capabilities STRling advertises.
"""
...
class JsonRPCServer:
    ...
    def feature(
        ...
        def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
            # store a reference for potential introspection. We also keep
            # the registration options (e.g. SemanticTokensLegend) so the
            # capability negotiation layer can surface them on initialize.
