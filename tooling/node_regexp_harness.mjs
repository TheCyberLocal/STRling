...
function boundedCase(rawCase) {
    ...
    const mode =
        item.mode === undefined
            ? "exec"
            : requireString(item.mode, `${id}.mode`);
    ...
    if (
        subjects.some((subject) => subject.length > MAXIMUM_SUBJECT_CODE_UNITS)
    ) {
        throw new RangeError(`${id}.subject exceeds the UTF-16 limit`);
    }
    const captureNames =
        item.capture_names === undefined
            ? {}
    ...
    const maximumMatches =
        item.maximum_matches === undefined ? 1 : item.maximum_matches;
    if (
        !Number.isSafeInteger(maximumMatches) ||
        maximumMatches < 1 ||
        maximumMatches > MAXIMUM_MATCHES
    ) {
...
function assertParity(baseline, instrumented, caseId) {
    if ((baseline === null) !== (instrumented === null)) {
        throw new Error(
            `${caseId}: index instrumentation changed match presence`,
        );
    }
    if (baseline === null) {
        return;
    ...
    if (
        baseline.index !== instrumented.index ||
        baseline.length !== instrumented.length
    ) {
    ...
    for (let index = 0; index < baseline.length; index += 1) {
        if (baseline[index] !== instrumented[index]) {
            throw new Error(
                `${caseId}: index instrumentation changed capture values`,
            );
        }
    }
}
    ...
    while (true) {
        ...
        if (matches.length >= item.maximumMatches) {
            throw new RangeError(
                `${item.id}: match count exceeds the governed limit`,
            );
        }
        ...
        if (detailed[0] === "") {
            const next = advanceStringIndex(
                subject,
                instrumented.lastIndex,
                unicode,
            );
...
try {
    ...
    process.stderr.write(
        `${JSON.stringify({ protocol_version: PROTOCOL_VERSION, fatal: { code: "HARNESS_ERROR", name } })}\n`,
    );
    process.exitCode = 1;
}
