...
GENERIC_HINT_FALLBACK <- "Check the STRling documentation for help with this syntax."
...
#' Get a hint for the given error (dynamic generators).
#' @param error_message The error message from the parser
#' @param text The full input text being parsed
#' @param pos The position where the error occurred (1-indexed)
#' @return A helpful hint string, or NULL if no hint is available
.hint_unexpected_token <- function(msg, text, pos) {
  if (!is.null(pos) && !is.null(text) && pos <= nchar(text)) {
...
.hint_unknown_escape <- function(msg, text, pos) {
  m <- regmatches(msg, regexpr("Unknown escape sequence \\\\?(.)", msg, perl = TRUE))
  if (length(m) > 0) {
    ch <- sub(".*\\\\(.)", "\\1", m[1])
    if (ch == "z") {
      return("'\\z' is not a recognized escape sequence. Did you mean '\\Z' (end of string) or escape the literal 'z' as 'z'?")
    }
    return(paste0("Unknown escape sequence '\\", ch, "'. If you intended a literal '", ch, "', remove the backslash or use a recognized escape."))
  }
  "This is not a recognized escape sequence."
}

.hint_invalid_quantifier <- function(msg, text, pos) {
  m <- regmatches(msg, regexpr("Invalid quantifier '(.)'", msg, perl = TRUE))
  ch <- if (length(m) > 0) sub(".*'(.)'.*", "\\1", m[1]) else "*"
  paste0("The quantifier '", ch, "' must follow an atom (a character or group). Place '", ch, "' after the thing it should quantify, e.g., 'a", ch, "'.")
}
...
#' @return A helpful hint string, or NULL if no hint is available
#' @export
get_hint <- function(error_message, text = "", pos = 0) {
  if (is.null(error_message)) return(NULL)

  # Check static hints (substring match)
  for (pattern in names(.hint_map)) {
    if (grepl(pattern, error_message, fixed = TRUE)) {
      return(.hint_map[[pattern]])
    }
  }

  # Check dynamic generators
  if (grepl("Unexpected token", error_message, fixed = TRUE)) {
    return(.hint_unexpected_token(error_message, text, pos))
  }
  if (grepl("Unknown escape sequence", error_message, fixed = TRUE)) {
    return(.hint_unknown_escape(error_message, text, pos))
  }
  if (grepl("Invalid quantifier", error_message, fixed = TRUE)) {
    return(.hint_invalid_quantifier(error_message, text, pos))
  }
...
#' @param text The full input text being parsed
#' @param pos The position where the error occurred
#' @return A helpful hint string (never NULL)
#' @export
get_hint_or_fallback <- function(error_message, text = "", pos = 0) {
  hint <- get_hint(error_message, text, pos)
  if (is.null(hint)) GENERIC_HINT_FALLBACK else hint
}
