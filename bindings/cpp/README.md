...
Here is how to match a US Phone number (e.g., `555-0199`) using the new `simply` fluent API from STRling in **C++ (C++17)**:
...
int main() {
    ...
    std::string regex = phone.compile();
}
```

> **Note:** The example above contains zero raw pointer usage — `std::unique_ptr`, `new`, and `std::make_shared` are all absent from user-facing code.
