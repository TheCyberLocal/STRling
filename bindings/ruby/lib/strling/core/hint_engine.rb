...
module Strling
  module Core
    ...
    def self.get_hint(message, text, pos)
      ...
      STATIC_HINTS.each do |pattern, hint|
        return hint if message.include?(pattern)
      end

      # Dynamic generators
      if message.include?("Unexpected token")
        return hint_unexpected_token(message, text, pos)
      end
      if message.include?("Unknown escape sequence")
        return hint_unknown_escape(message, text, pos)
      end
      if message.include?("Invalid quantifier")
        return hint_invalid_quantifier(message, text, pos)
      end
    ...
    def self.get_hint_or_fallback(message, text, pos)
      get_hint(message, text, pos) || GENERIC_HINT_FALLBACK
    end

    def self.hint_unexpected_token(msg, text, pos)
      if pos && text && pos < text.length
        ch = text[pos]
        if ch == ')'
    ...
    private_class_method :hint_unexpected_token

    def self.hint_unknown_escape(msg, text, pos)
      m = msg.match(/Unknown escape sequence \\?(.)/m)
      if m
        ch = m[1]
        if ch == 'z'
    ...
    private_class_method :hint_unknown_escape

    def self.hint_invalid_quantifier(msg, text, pos)
      m = msg.match(/Invalid quantifier '(.)'/)
      ch = m ? m[1] : '*'
      "The quantifier '#{ch}' must follow an atom (a character or group). Place '#{ch}' after the thing it should quantify, e.g., 'a#{ch}'."
    end
    private_class_method :hint_invalid_quantifier
  end
end
