...
/* Port of bindings/python/src/STRling/core/nodes.py to C (data structures only) */
#include "nodes.h"
#include <stdlib.h>
#include <string.h>
...
void strling_ast_node_free(STRlingASTNode* node) {
    ...
    switch (node->type) {
        ...
        case AST_TYPE_CHARCLASS:
            ...
            break;
        case AST_TYPE_QUANT:
            if (node->u.quant.child) strling_ast_node_free(node->u.quant.child);
            free(node->u.quant.mode);
            break;
        case AST_TYPE_GROUP:
            if (node->u.group.body) strling_ast_node_free(node->u.group.body);
            free(node->u.group.name);
            break;
        case AST_TYPE_BACKREF:
            free(node->u.backref.byName);
        ...
        case AST_TYPE_LOOK:
            free(node->u.look.dir);
            if (node->u.look.body) strling_ast_node_free(node->u.look.body);
            break;
    }
    free(node);
}
